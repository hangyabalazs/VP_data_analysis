function slowanalysis_pavlovian(vpcells, preprocess, fig1spec, fig2spec, ...
    fig3spec, fig4spec, fig5spec, fig6spec, fig7spec, fig8spec)
%SLOWANALYSIS_PAVLOVIAN   Analysis main function for VP project.
%   SLOWANALYSIS_PAVLOVIAN(VPCELLS, PRE, F1, F2, ..., F8) is the main
...
% Directories
resdir = [getpref('cellbase','datapath') '\_paper_figs\code_review\']; % directory for generated images
if ~isfolder(resdir)
    mkdir(resdir)
end
...
if lick_stat
    ...
    anticipatory_stat_VP('HDB13',resdir_astat);
    anticipatory_stat_VP('HDB17',resdir_astat);
    anticipatory_stat_VP('HDB25',resdir_astat);
    anticipatory_stat_VP('HDB32',resdir_astat);
end
...
if box_stat
    ...
    [H1, Wp2] = boxstat(latencies.cue.excited.latency_peak,latencies.reward.excited.latency_peak,'cue','reward', 0.005); %#ok<*ASGLU>
...
if pie_PSTH_bursting
    ...
    % Pie charts - FIG.4B-C
    load(fullfile(getpref('cellbase','datapath'), '_paper_figs\code_review\ACG_matrices.mat'));  %#ok<*LOAD> % load ACG matrices
    ...
    resdir_avg_psth_VP1 = fullfile(resdir,'fig4','bursting');
    ...
    resdir_avg_psth_VP2 = fullfile(resdir,'fig4','nonbursting');
    ...
    % Maxvalue stats
    resdir_maxval_comp_bursting = fullfile(resdir,'fig4','bursting_maxval');
...
if pie_PSTH_rhythmic
    ...
    [phasic_cells, BetaIndex, GammaIndex] = pr_freq_detection_VP(vpcells,resdir_acg_rhythmic,resdir2,resdir3,acgdir,responsedir);
    beta_cells = phasic_cells(BetaIndex > 0.4); % beta cells - value was empirical
    gamma_cells = phasic_cells(GammaIndex > 0.25); % gamma cells
...
if select_pairs
    most_pairs = select_monosyn_pairs(ccgdir); %#ok<NASGU> % only part of the connections are shown in the fig.
end
...
if norhythmandburst  % FIG.8K - select non-bursting- non-rhythmic cells and compare their synchrony to others
    ...
    load(fullfile(resdir_acg_rhythmic,'phasic_response.mat'));
    rhythmic_cells = phasic_cells(BetaIndex > 0.4 | GammaIndex > 0.25);  % rhythmic neurons
    rhythmic_cells = unique(rhythmic_cells);   % beta/gamma may overlap
    nonrhythmic_cells = setdiff(vpcells,rhythmic_cells);  % non-rhythmic VP neurons
