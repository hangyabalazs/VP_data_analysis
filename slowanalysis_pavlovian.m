...
% Directories
resdir = [getpref('cellbase','datapath') '\_paper_figs\code_review2\']; % directory for generated images
if ~isfolder(resdir)
    mkdir(resdir)
end
...
if pie_PSTH_bursting
    ...
    % Pie charts - FIG.4B-C
    load(fullfile(getpref('cellbase','datapath'), '_paper_figs\code_review\ACG_matrices.mat'));  % load ACG matrices (generated by vpacg.m)
...
if norhythmandburst  % FIG.8K - select non-bursting- non-rhythmic cells and compare their synchrony to others
    load(fullfile(acgdir,'ACG_matrices_.mat'));  % load ACG matrices
